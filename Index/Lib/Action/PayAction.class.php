	...
	*/
	class PayAction extends Action {
		...
		function index(){
            ...
            $tongji = array();
            //取得所需消费易得币
            foreach ($_POST['yidebi'] as $item => $vo){
               $tongji['totalyidebi']+= $vo;
            }
            //取得所需消费积分
            foreach ($_POST['jifen'] as $item => $vo){
                $tongji['totaljifen']+= $vo;
            }
            //先判断该用户的积分和易得币是否满足条件
            $userjifenyide = M('user_jifenyide');
            $userjfydData = $userjifenyide->where(array('uid' => $_SESSION['uid']))->find();
            if($userjfydData['jifen']<$tongji['totaljifen']){
                $this->error('积分不够！');
            }
            if($userjfydData['yide']<$tongji['totalyidebi']){
                $this->error('易得币不够！');
            }
			...
			//订单所需积分
            if(empty(trim($tongji['totaljifen'])))
            {
                $dingdan['totaljifen'] = 0;
            }
            else{
                $dingdan['totaljifen'] = trim($tongji['totaljifen']);
            }
            //订单所需易得币
            if(empty(trim($tongji['totalyidebi'])))
            {
                $dingdan['totalyidebi'] = 0;

            }
            else{
                $dingdan['totalyidebi'] = trim($tongji['totalyidebi']);
            }
			...
			for($i = 0; $i < $length;$i++){
                ...
                //商品积分
                $dsp['jifen']= $_POST['jifen'][$i];
                //商品易得币
                $dsp['yidebi']= $_POST['yidebi'][$i];
                //商品小计
